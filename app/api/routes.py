...
async def remove_background(
    ...
    try:
        ...
        if not remover_service.is_ready:
            raise HTTPException(status_code=503, detail="Background remover service not ready")
        ...
        with PerformanceLogger("Single image API processing", request_id):
            ...
            # Convert to bytes
            format_to_use = output_format.upper() if output_format else settings.DEFAULT_OUTPUT_FORMAT
...
async def remove_background_batch(
    ...
    try:
        ...
        with PerformanceLogger(f"Batch API processing ({len(files)} files)", request_id):
            # Load all images
            images = []
            filenames = []
            ...
            logger.info(f"Loaded {len(images)} images for batch processing [req: {request_id}]")
            
            # Process batch
            result_images = await remover_service.remove_background_batch(images, request_id)
            ...
            with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
                for i, (result_image, original_filename) in enumerate(zip(result_images, filenames)):
            ...
            logger.info(f"Successfully processed batch of {len(files)} images [req: {request_id}]")
...
async def list_available_models():
    ...
    return {
        ...
        "current_model_info": models.get(current_model, {"name": "Unknown", "description": "Custom model"})
    }
